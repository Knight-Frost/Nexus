  ...
  if (contractsRes.status === 200) {
    try {
      ...
      if (Array.isArray(contracts)) {
        ...
        console.log('⚠️  Contracts response is not an array:', typeof contracts);
      }
    } catch (e) {
      console.log('⚠️  Error parsing contracts:', e);
    }
  } else {
    console.log('⚠️  Contracts request failed:', contractsRes.status);
  }
  ...
  if (ledgerRes.status === 200) {
    try {
      ...
      if (Array.isArray(entries)) {
        ...
        console.log('⚠️  Ledger response is not an array:', typeof entries);
      }
    } catch (e) {
      console.log('⚠️  Error parsing ledger:', e);
    }
  } else {
    console.log('⚠️  Ledger request failed:', ledgerRes.status);
  }
  
  console.log(`✅ Loaded ${contractIds.length} contracts, ${ledgerIds.length} ledger entries`);
  
  if (contractIds.length === 0) {
    throw new Error('❌ No contracts found - ensure server is running and data is seeded');
  }
